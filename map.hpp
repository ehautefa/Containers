...
/*   Updated: 2022/03/16 21:03:19 by ehautefa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	private:
		...
		void erase_root()
		{
			...
			else if (_root->_left == _rend || _root->_right == _end)
			{ // CHILD LEFT OR RIGHT ONLY
		...
		void erase_node(node_type *to_erase)
		{
			...
			if ((!to_erase->_left || to_erase->_left == _rend) && (!to_erase->_right || to_erase->_right == _end))
			{ // NO CHILD
			...
			else if (((!to_erase->_left || to_erase->_left == _rend)) || (!to_erase->_right || to_erase->_right == _rend))
			{ // ONLY ONE CHILD
	...
	{
		typename ft::map<Key, T, Compare, Alloc>::const_iterator l_beg = lhs.begin();
		typename ft::map<Key, T, Compare, Alloc>::const_iterator r_beg = rhs.begin();
		typename ft::map<Key, T, Compare, Alloc>::const_iterator l_end = lhs.end();
		if (lhs.size() == rhs.size() && ft::equal(l_beg, l_end, r_beg))
			return true;
		return false;
	}
	template <class Key, class T, class Compare, class Alloc>
